...
- [ ] T4 (10m): Monitor CI run and classify current failure reason with evidence.
- [ ] T5 (10m): Wire Provider config UI -> `save_provider_config` + `load_config`.
- [ ] T6 (10m): Wire Agent config UI -> `save_agent_config` + `load_config`.
- [ ] T7 (10m): Wire Channel config UI -> `save_channel_config` + `validate_channel`.
